...
def get_prediction(data: dict) -> dict:
    ...
    data_df['purchase_timestamp'] = data_df['purchase_timestamp'].apply(
        lambda x: pd.to_datetime(x))
    data_df['delivery_weekday'] = data_df['purchase_timestamp'].apply(
        lambda x: ls[x.weekday()])
    data_df['purchase_morning'] = data_df['purchase_timestamp'].apply(
        lambda x: 1 if x.hour <= 14 else 0)
    if data_df['purchase_morning'][0] == 1:
        flag_morning = 1
    data_df.drop(["purchase_timestamp", "purchase_morning"],
                 axis=1,
                 inplace=True)

    final_df = {}
    column_names = [
        'purchase_morning', 'Kraków', 'Poznań', 'Radom', 'Szczecin',
        'Warszawa', 'Wrocław', 360, 516, 620, 'monday', 'saturday', 'sunday',
